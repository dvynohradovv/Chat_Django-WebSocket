...
class EditChatRoomView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        user = request.user
        ...
        return render(request, 'chat/room-edit.html', context)

    def post(self, request, *args, **kwargs):
        user = request.user
        room_name = kwargs["room_name"]

        sendto_username = request.POST["username"]

        user = User.objects.filter(username=sendto_username)
        if user.exists():
            user = user.get(username=sendto_username)
            chatroom = Chatroom.objects.get(name=room_name)
            user.chatroom_invitations.add(chatroom)
            user.save()
            result = "Successfuly!"
        else:
